...
class HandwrittenDigitRecognizer:
    def __init__(self, model_path: str = "models/digit_recognizer_model.h5"):
        """Initialize the HandwrittenDigitRecognizer with model path."""
        self.model_path = model_path
        self.model = None
        self.class_names = [str(i) for i in range(10)]  # Digits 0-9
        self.img_height, self.img_width = 28, 28  # MNIST image size
    ...
    def load_data(self) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray]]:
        ...
        try:
            ...
            logger.info(f"Loaded MNIST dataset: Train {train_images.shape}, Test {test_images.shape}")
    ...
    def build_model(self) -> Sequential:
        """Build the CNN model for digit recognition with BatchNormalization."""
        model = Sequential([
            Conv2D(32, (3, 3), activation='relu', input_shape=(self.img_height, self.img_width, 1), padding='same'),
            BatchNormalization(),
        ...
        logger.info("Model built and compiled with BatchNormalization.")
        return model

    def train_model(self, epochs: int = 50, batch_size: int = 64, validation_split: float = 0.2) -> None:
        """Train the model on the MNIST dataset with data augmentation."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Handwritten Digit Recognizer using CNN on MNIST.")
    parser.add_argument("--mode", type=str, choices=["train", "predict"], required=True,
                        help="Mode: 'train' to train model, 'predict' for inference on an image.")
    parser.add_argument("--model_path", type=str, default="models/digit_recognizer_model.h5", help="Path to save/load the model.")
...
if __name__ == "__main__":
    ...
    recognizer = HandwrittenDigitRecognizer(model_path=args.model_path)
    
    if args.mode == "train":
        recognizer.train_model(epochs=args.epochs, batch_size=args.batch_size)
    elif args.mode == "predict":
        if not args.image_path:
            logger.error("Image path required for predict mode.")
            sys.exit(1)
        prediction = recognizer.predict(args.image_path)
        if prediction:
