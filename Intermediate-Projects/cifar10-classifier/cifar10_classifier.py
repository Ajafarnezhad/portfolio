...
class CIFAR10Classifier:
    def __init__(self, model_path: str = "models/cifar10_classifier_model.h5"):
        """Initialize the CIFAR10Classifier with model path."""
        self.model_path = model_path
        self.model = None
        self.class_names = ['airplane', 'automobile', 'bird', 'cat', 'deer',
                           'dog', 'frog', 'horse', 'ship', 'truck']
        self.img_height, self.img_width = 32, 32  # CIFAR-10 image size
    ...
    def load_data(self) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray]]:
        ...
        try:
            ...
            # Reshape for CNN (already 3 channels for CIFAR-10)
            train_labels = tf.keras.utils.to_categorical(train_labels, num_classes=10)
            test_labels = tf.keras.utils.to_categorical(test_labels, num_classes=10)
            
            logger.info(f"Loaded CIFAR-10 dataset: Train {train_images.shape}, Test {test_images.shape}")
    ...
    def build_model(self) -> Sequential:
        """Build the CNN model for CIFAR-10 classification."""
        model = Sequential([
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="CIFAR-10 Image Classifier using CNN.")
    parser.add_argument("--mode", type=str, choices=["train", "predict"], required=True,
                        help="Mode: 'train' to train model, 'predict' for inference on an image.")
    parser.add_argument("--model_path", type=str, default="models/cifar10_classifier_model.h5", help="Path to save/load the model.")
...
if __name__ == "__main__":
    ...
    classifier = CIFAR10Classifier(model_path=args.model_path)
